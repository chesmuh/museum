...
public class CreateMuseumComposite extends Composite {

	private Text textName;
	private Text textDescription;

	public CreateMuseumComposite(Composite parent) {
	...
	private void initialize() {
		...
		label.setText(ResourceManager
				.getText(OrdoUI.DETAIL_MUSEUM_DESCRIPTION));
		gridData = new GridData(SWT.RIGHT, SWT.CENTER, false, true);
		label.setLayoutData(gridData);
	...
	private class SaveSelectionListener extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			if (null == textName.getText()) {
				...
				try {
					Museum museum = LogicAccess.saveMuseum(textName.getText(), textDescription.getText());
	...
	private class CancelSelectionListener extends SelectionAdapter {
		...
		@Override
		public void widgetSelected(SelectionEvent e) {
			UiEvent event = new UiEvent(null, UiEventType.AddMuseumCanceled);
			MainFrame.handleEvent(event);
		}
	}
}
